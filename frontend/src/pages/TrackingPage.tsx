...
const PHASE1_STATUSES: RideStatus[] = [
  "REQUESTED",
  "DRIVER_ASSIGNED",
  "DRIVER_EN_ROUTE",
  "DRIVER_ARRIVED",
...
export default function TrackingPage() {
  ...
  useEffect(() => {
    ...
    getRide(rideId)
      .then((ride) => {
        // Treat REQUESTED and DRIVER_ASSIGNED as "searching" until driver actually starts heading
        const backendStatus = ride.status as RideStatus;
        const isPreAccept =
          backendStatus === "REQUESTED" || backendStatus === "DRIVER_ASSIGNED";
        const effectiveStatus: RideStatus = isPreAccept
          ? "REQUESTED"
          : backendStatus;
        ...
        dispatch({
          ...
          payload: {
            ...
            // Only show driver info if driver has actually started heading to pickup
            driverInfo:
              !isPreAccept && ride.driver_name
                ? {
                    id: ride.driver_id || "",
  ...
  useEffect(() => {
    if (
      ...
      fetchOSRMRoute(
        ...
        if (route.length > 1) setFullRoute(route);
      });
    }
  }, [
    state.trackingPhase,
  ...
  const onLocationUpdate = useCallback(
    (data: {
      ...
      setRoutePoints((prev) => {
        ...
        if (
          !last ||
          Math.abs(last[0] - data.lat) > 0.00001 ||
          Math.abs(last[1] - data.lng) > 0.00001
        ) {
    ...
    (data: {
      ...
      dispatch({
        ...
        payload: {
          ...
          trackingPhase: data.trackingPhase as TrackingPhase | undefined,
        },
      });
      ...
      // When driver accepts, re-fetch ride to get driver info
      if (
        ...
        getRide(rideId)
          .then((ride) => {
            if (ride.driver_name) {
              ...
              setToast({ visible: true, message: "Driver found! On the way." });
            }
          })
          .catch(console.error);
      }
  ...
  const onDriverArrived = useCallback(() => {
    dispatch({
      type: "UPDATE_STATUS",
      payload: { status: "DRIVER_ARRIVED", trackingPhase: "arrival" },
    });
  ...
  const onRideStarted = useCallback(() => {
    dispatch({
      type: "UPDATE_STATUS",
      payload: { status: "RIDE_ACTIVE", trackingPhase: "trip" },
    });
    setRoutePoints([]);
  ...
  const onRideCompleted = useCallback(
    (data: { rideId: string; totalFare: number; duration: string }) => {
  ...
  const onPinVerified = useCallback(() => {
    dispatch({
      type: "UPDATE_STATUS",
      payload: { status: "PIN_VERIFIED", trackingPhase: "trip" },
    });
  ...
  const isRideActive =
    state.status === "RIDE_ACTIVE" || state.status === "PIN_VERIFIED";
  ...
  const isSearching = state.status === "REQUESTED";
  ...
  return (
    <div className="relative w-[375px] h-[812px] bg-white overflow-hidden">
      ...
      {/* Route summary card */}
      <div
        className="relative z-30 mx-4 mt-2 p-3 bg-white rounded-xl"
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-[13px]" style={{ color: "#C8C7CC" }}>
            →
          </span>
      ...
      <div className="absolute top-[120px] left-0 right-0 bottom-[280px] z-10">
        {state.pickupLat !== 0 && (
          <MapLayer
            ...
            driverPos={
              state.driverLocation ? { ...state.driverLocation } : null
            }
            riderPos={
              state.trackingPhase === "arrival"
      ...
      >
        <div
          className="w-[50px] h-[5px] rounded-[3px] mx-auto mb-3"
        ...
        <div className="mb-3">
          ...
          {state.eta && !isSearching && (
            <p
              className="text-[12px]"
              style={{ color: "#8A8A8F", fontFamily: "Inter, sans-serif" }}
            >
              Est. Arrival: {state.eta}{" "}
              {state.distanceRemaining && `· ${state.distanceRemaining}`}
            </p>
          )}
          {isSearching && (
        ...
        {isDriverArrived && state.pin && state.status !== "PIN_VERIFIED" && (
          ...
          >
            <p
              className="text-[13px] text-center mb-2"
              style={{ color: "#8A8A8F" }}
            >
          ...
          >
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-1 mt-0.5">
                ...
                <span className="text-[12px]" style={{ color: "#242E42" }}>
            ...
            <div className="flex gap-2">
              <a
                href={`tel:${state.driverInfo.phone}`}
        ...
        {showModifyRoute && (
          <div className="mt-2 flex gap-2">
            ...
            <CTAButton
              ...
              className="!w-auto !h-[36px] px-4 !text-[12px]"
            />
          </div>
        )}
