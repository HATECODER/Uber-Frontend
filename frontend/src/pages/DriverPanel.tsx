...
import { getDriverRide, setRideStatus, driverVerifyPin, submitRating, getPendingRide } from '../api/rides';
...
type DriverStatus = 'OFFLINE' | 'ONLINE' | 'RIDE_REQUEST' | 'EN_ROUTE' | 'ARRIVED' | 'START_RIDE' | 'ACTIVE_RIDE' | 'COMPLETE' | 'RATE';
...
export default function DriverPanel() {
  ...
  const mapBackendStatus = useCallback((status: string): DriverStatus => {
    switch (status) {
      case 'REQUESTED': return 'RIDE_REQUEST';
      case 'DRIVER_ASSIGNED':
      case 'DRIVER_EN_ROUTE': return 'EN_ROUTE';
      case 'DRIVER_ARRIVED': return 'ARRIVED';
      case 'PIN_VERIFIED': return 'START_RIDE';
      case 'RIDE_ACTIVE': return 'ACTIVE_RIDE';
      case 'DESTINATION_REACHED': return 'COMPLETE';
      case 'RATING': return 'RATE';
      case 'COMPLETED': return 'RATE';
      default: return 'ONLINE';
    }
  }, []);

  // Socket listeners — driver receives its own relayed location from backend
  const onLocationUpdate = useCallback((data: { lat: number; lng: number; heading: number; speed: number; eta: string }) => {
  ...
  const onProgressUpdate = useCallback((data: { eta: string; distanceRemaining: string; trackingPhase: string }) => {
    setEta(data.eta || '');
  ...
  const onStatusChanged = useCallback((data: { status: string }) => {
  ...
  const onRideStarted = useCallback(() => { setDriverStatus('ACTIVE_RIDE'); setRoutePoints([]); }, []);
  const onRideCompleted = useCallback((data: { totalFare: number; duration: string }) => {
  ...
  const buildSimPoints = useCallback(async (
  ...
  const handleAccept = async () => {
    ...
    try {
      ...
      simulationRef.current = startSimulation(
        ...
        () => {
          // Arrival simulation complete → emit arrived_at_pickup
  ...
  const handleDecline = async () => {
    ...
    stopSim();
    stopPolling();
    try { await setRideStatus(ride.id, 'CANCELLED'); } catch {}
    setRide(null);
    setDriverStatus('ONLINE');
  };
  ...
  const handleCompleteTrip = async () => {
    ...
    } catch { /* already at correct state */ }
  ...
  const trackingPhase = useMemo(() => {
    if (driverStatus === 'ACTIVE_RIDE' || driverStatus === 'COMPLETE') return 'trip' as const;
    ...
    <div className="relative w-[375px] h-[812px] bg-white overflow-hidden">
      ...
      <div className="relative z-50 bg-white">
        ...
        <div className="flex items-center justify-between px-4 pb-2">
          <span className="text-[24px] font-bold" style={{ color: '#242E42', fontFamily: 'Poppins, sans-serif' }}>
            &#2547;{ride?.finalFare || ride?.estimatedFare || '—'}
          </span>
          {ride && (
            <span className="text-[9px] px-1.5 py-0.5 rounded bg-gray-100 font-mono truncate max-w-[120px]" style={{ color: '#8A8A8F' }}>
              {ride.id.slice(0, 8)}
            </span>
          )}
          {ride && (
            <span className="text-[10px] px-2 py-0.5 rounded-full font-semibold" style={{ background: routeSource === 'osrm' ? '#4CE5B1' : '#FFD700', color: '#242E42' }}>
      ...
      {(driverStatus === 'ONLINE' || driverStatus === 'RIDE_REQUEST') && (
        <div className="relative z-40 px-4 mb-2">
          <button
            onClick={() => { stopPolling(); if (ride) handleDecline(); else setDriverStatus('OFFLINE'); }}
            className="w-full h-[44px] rounded-full text-white text-[16px] font-semibold"
            style={{ background: 'linear-gradient(135deg, #FF6B6B, #EE5A24)', border: 'none', cursor: 'pointer' }}
          >
            Go Offline
          </button>
        </div>
      ...
      {ride && (driverStatus === 'EN_ROUTE' || driverStatus === 'ARRIVED' || driverStatus === 'START_RIDE') && (
        <div className="relative z-30 mx-4 mb-2 p-3 bg-white rounded-xl" style={{ boxShadow: '0 2px 8px rgba(0,0,0,0.08)' }}>
          <div className="flex items-start gap-2 mb-2">
            <div className="mt-1">
              <div className="w-[10px] h-[10px] rounded-full" style={{ background: '#4CE5B1' }} />
              <div className="w-[1px] h-[20px] mx-auto" style={{ background: '#CCCCCC' }} />
              <div className="w-[10px] h-[10px] rounded-full" style={{ background: '#4CE5B1' }} />
            </div>
      ...
      <div
        ...
        style={{
          top: driverStatus === 'OFFLINE' || driverStatus === 'ONLINE' ? '90px' : (ride && (driverStatus === 'EN_ROUTE' || driverStatus === 'ARRIVED' || driverStatus === 'START_RIDE') ? '220px' : '130px'),
          bottom: driverStatus === 'OFFLINE' ? '200px' : (driverStatus === 'COMPLETE' ? '200px' : '180px'),
        }}
      >
        <MapLayer
          center={mapCenter}
        ...
        <button
          ...
          style={{ boxShadow: '0 2px 6px rgba(0,0,0,0.15)', border: 'none', cursor: 'pointer' }}
        >
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#4CE5B1" strokeWidth="2">
      ...
      <div className="absolute bottom-0 left-0 right-0 z-40" style={{ background: 'rgba(255,255,255,0.98)', borderRadius: '16px 16px 0 0', boxShadow: '0px -5px 10px rgba(0,0,0,0.10)' }}>
        <div className="w-[50px] h-[5px] rounded-[3px] mx-auto mt-2 mb-2" style={{ background: '#CCCCCC' }} />

        {/* OFFLINE */}
        {driverStatus === 'OFFLINE' && (
          <div className="px-4 pb-6">
            <p className="text-center text-[18px] font-semibold mb-4" style={{ color: '#F52D56' }}>You're Offline</p>
            <div className="flex items-center gap-3 mb-4">
              <div className="w-[50px] h-[50px] rounded-full bg-gray-200 flex items-center justify-center overflow-hidden">
                <svg width="28" height="28" viewBox="0 0 24 24" fill="#8A8A8F"><path d="M12 12a5 5 0 100-10 5 5 0 000 10zm0 2c-5.33 0-8 2.67-8 4v2h16v-2c0-1.33-2.67-4-8-4z" /></svg>
              </div>
              <div className="flex-1">
                <p className="text-[16px] font-semibold" style={{ color: '#242E42' }}>Ramlal</p>
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-[13px]" style={{ color: '#4CE5B1' }}>4.9</span>
                </div>
              </div>
              <button
                ...
                style={{ background: '#4CE5B1', border: 'none', cursor: 'pointer' }}
              >
                {loading === 'online' ? '...' : 'Go'}
              </button>
            </div>
            {/* Stats row */}
            <div className="flex justify-around pt-3" style={{ borderTop: '1px solid #E8E8E8' }}>
              <div className="text-center">
                <div className="flex items-center justify-center gap-1">
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="#4CE5B1"><path d="M9 16.2L4.8 12l-1.4 1.4L9 19 21 7l-1.4-1.4L9 16.2z" /></svg>
                  <span className="text-[16px] font-bold" style={{ color: '#242E42' }}>95.0%</span>
                </div>
                <span className="text-[12px]" style={{ color: '#8A8A8F' }}>Acceptance</span>
              </div>
              <div className="text-center">
                <div className="flex items-center justify-center gap-1">
                  ...
                  <span className="text-[16px] font-bold" style={{ color: '#242E42' }}>4.9</span>
                </div>
                <span className="text-[12px]" style={{ color: '#8A8A8F' }}>Rating</span>
              </div>
              <div className="text-center">
                <div className="flex items-center justify-center gap-1">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="#F52D56"><rect x="4" y="4" width="16" height="16" rx="3" /></svg>
                  <span className="text-[16px] font-bold" style={{ color: '#242E42' }}>2.0%</span>
                </div>
                <span className="text-[12px]" style={{ color: '#8A8A8F' }}>Cancellation</span>
              </div>
            </div>
            {/* Quick load ride */}
            <button onClick={() => setShowRideInput(!showRideInput)} className="mt-3 text-[12px] underline w-full text-center" style={{ color: '#8A8A8F', background: 'none', border: 'none', cursor: 'pointer' }}>
              Load existing ride by ID
            </button>
            {showRideInput && (
              <div className="mt-2 flex gap-2">
                <input value={rideIdInput} onChange={(e) => setRideIdInput(e.target.value)} placeholder="Ride ID" className="flex-1 h-[36px] px-3 rounded-lg text-[13px]" style={{ background: '#F7F4F4', border: '1px solid #CCCCCC' }} />
                <button onClick={() => { handleLoadRide(rideIdInput); setShowRideInput(false); }} className="h-[36px] px-4 rounded-lg text-white text-[13px] font-semibold" style={{ background: '#5856D6', border: 'none', cursor: 'pointer' }}>Load</button>
              </div>
            )}
          </div>
        )}
        ...
        {driverStatus === 'ONLINE' && (
          <div className="px-4 pb-6 text-center">
            <p className="text-[18px] font-bold mb-1" style={{ color: '#242E42' }}>You're Online</p>
            <p className="text-[16px] font-semibold mb-1" style={{ color: '#4CE5B1' }}>Finding Trips</p>
            <p className="text-[14px] mb-2" style={{ color: '#8A8A8F' }}>Waiting for rider to book a ride...</p>
            <div className="flex justify-center">
              <div className="w-6 h-6 border-2 border-t-transparent rounded-full animate-spin" style={{ borderColor: '#4CE5B1', borderTopColor: 'transparent' }} />
            </div>
          </div>
        )}
        ...
        {driverStatus === 'RIDE_REQUEST' && ride && (
          <div className="px-4 pb-4">
            <div className="flex justify-center mb-2">
              <div className="w-[40px] h-[40px] rounded-full flex items-center justify-center" style={{ background: '#F0F0F0' }}>
                <svg width="22" height="22" viewBox="0 0 24 24" fill="#242E42"><path d="M18.92 6.01C18.72 5.42 18.16 5 17.5 5h-11c-.66 0-1.21.42-1.42 1.01L3 12v8c0 .55.45 1 1 1h1c.55 0 1-.45 1-1v-1h12v1c0 .55.45 1 1 1h1c.55 0 1-.45 1-1v-8l-2.08-5.99zM6.5 16c-.83 0-1.5-.67-1.5-1.5S5.67 13 6.5 13s1.5.67 1.5 1.5S7.33 16 6.5 16zm11 0c-.83 0-1.5-.67-1.5-1.5s.67-1.5 1.5-1.5 1.5.67 1.5 1.5-.67 1.5-1.5 1.5zM5 11l1.5-4.5h11L19 11H5z" /></svg>
              </div>
            </div>
            <p className="text-center text-[20px] font-bold mb-1" style={{ color: '#4CE5B1' }}>NEW RIDE REQUEST</p>
            <p className="text-center text-[14px] mb-3" style={{ color: '#8A8A8F' }}>
              {eta} <span className="mx-1">&#128100;</span> {distance}
            </p>
            <div className="flex items-center gap-2 mb-4">
              <div className="w-[10px] h-[10px] rounded-full" style={{ background: '#4CE5B1' }} />
              <div>
                <p className="text-[12px]" style={{ color: '#8A8A8F' }}>Pick up: {ride.pickupAddress}</p>
                <p className="text-[14px] font-semibold" style={{ color: '#242E42' }}>{ride.destAddress}</p>
              </div>
            </div>
            <div className="flex gap-3">
              <button onClick={handleDecline} className="flex-1 h-[48px] rounded-full text-white text-[16px] font-semibold" style={{ background: 'linear-gradient(135deg, #FF6B6B, #EE5A24)', border: 'none', cursor: 'pointer' }}>
                DECLINE
              </button>
              <button onClick={handleAccept} disabled={loading === 'accept'} className="flex-1 h-[48px] rounded-full text-white text-[16px] font-semibold" style={{ background: 'linear-gradient(135deg, #FF6B6B, #EE5A24)', border: 'none', cursor: 'pointer' }}>
        ...
        {driverStatus === 'EN_ROUTE' && ride && (
          <div className="px-4 pb-4">
            <div className="text-center mb-3">
              <span className="text-[22px] font-bold" style={{ color: '#242E42' }}>{eta}</span>
              <span className="mx-2">&#128100;</span>
              <span className="text-[22px] font-bold" style={{ color: '#242E42' }}>{distance}</span>
            </div>
            <p className="text-center text-[14px] mb-4" style={{ color: '#8A8A8F' }}>Picking up {ride.riderName}</p>
            <div className="flex justify-center gap-6 mb-4">
              <div className="flex flex-col items-center">
                <div className="w-[44px] h-[44px] rounded-full flex items-center justify-center" style={{ background: '#4CE5B1' }}>
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="white"><path d="M6.62 10.79a15.053 15.053 0 006.59 6.59l2.2-2.2a1 1 0 011.01-.24c1.12.37 2.33.57 3.58.57a1 1 0 011 1V20a1 1 0 01-1 1A17 17 0 013 4a1 1 0 011-1h3.5a1 1 0 011 1c0 1.25.2 2.46.57 3.58a1 1 0 01-.24 1.01l-2.21 2.2z" /></svg>
                </div>
                <span className="text-[12px] mt-1" style={{ color: '#242E42' }}>Call</span>
              </div>
              <div className="flex flex-col items-center">
                <div className="w-[44px] h-[44px] rounded-full flex items-center justify-center" style={{ background: '#E8E8E8' }}>
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="#242E42"><path d="M20 2H4a2 2 0 00-2 2v18l4-4h14a2 2 0 002-2V4a2 2 0 00-2-2z" /></svg>
                </div>
                <span className="text-[12px] mt-1" style={{ color: '#242E42' }}>Message</span>
              </div>
              <div className="flex flex-col items-center">
                <div className="w-[44px] h-[44px] rounded-full flex items-center justify-center" style={{ background: '#E8E8E8' }}>
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="#242E42"><path d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z" /></svg>
                </div>
                <span className="text-[12px] mt-1" style={{ color: '#242E42' }}>Cancel Trip</span>
              </div>
            </div>
            <button onClick={handleArrived} disabled={!!loading} className="w-full h-[48px] rounded-full text-white text-[16px] font-semibold" style={{ background: '#4CE5B1', border: 'none', cursor: 'pointer' }}>
        ...
        {driverStatus === 'ARRIVED' && ride && (
          <div className="px-4 pb-4">
            ...
            <div className="flex justify-center gap-6 mb-4">
              ...
              <div className="flex flex-col items-center" onClick={handleCancelTrip} style={{ cursor: 'pointer' }}>
                <div className="w-[44px] h-[44px] rounded-full flex items-center justify-center" style={{ background: '#E8E8E8' }}>
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="#242E42"><path d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z" /></svg>
                </div>
                <span className="text-[12px] mt-1" style={{ color: '#242E42' }}>Cancel Trip</span>
            ...
            {/* PIN Input */}
            <div className="mb-3">
              <p className="text-center text-[13px] mb-2" style={{ color: '#8A8A8F' }}>Enter rider's 4-digit PIN</p>
              <input
                type="text"
                maxLength={4}
                value={pinInput}
                ...
                style={{ background: '#F7F4F4', border: '1px solid #CCCCCC', fontFamily: 'Poppins, sans-serif' }}
              />
              {pinError && <p className="text-[12px] mt-1 text-center" style={{ color: '#F52D56' }}>{pinError}</p>}
            </div>
            <button onClick={handleVerifyAndStart} disabled={!!loading || pinInput.length !== 4} className="w-full h-[48px] rounded-full text-white text-[16px] font-semibold" style={{ background: pinInput.length === 4 ? '#4CE5B1' : '#C8C7CC', border: 'none', cursor: 'pointer' }}>
        ...
        {driverStatus === 'START_RIDE' && ride && (
          <div className="px-4 pb-4">
            ...
            <p className="text-center text-[14px] mb-4" style={{ color: '#8A8A8F' }}>Picking up {ride.riderName}</p>
            <button onClick={handleStartRide} disabled={!!loading} className="w-full h-[48px] rounded-full text-white text-[16px] font-semibold" style={{ background: '#4CE5B1', border: 'none', cursor: 'pointer' }}>
        ...
        {driverStatus === 'ACTIVE_RIDE' && ride && (
          <div className="px-4 pb-4">
            <div className="flex items-center justify-between">
              <p className="text-[16px] font-semibold" style={{ color: '#242E42' }}>{navInstruction || 'Navigating...'}</p>
              <button onClick={handleCompleteTrip} className="text-[22px]" style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#4CE5B1' }}>
                &#10132;
              </button>
            </div>
          </div>
        ...
        {driverStatus === 'COMPLETE' && ride && (
          <div className="px-4 pb-4 text-center">
            <p className="text-[18px] font-bold mb-2" style={{ color: '#4CE5B1' }}>RIDE COMPLETE</p>
            <p className="text-[36px] font-bold mb-2" style={{ color: '#4CE5B1' }}>&#2547;{ride.finalFare || ride.estimatedFare}</p>
            <p className="text-[14px] mb-1" style={{ color: '#8A8A8F' }}>Time: {tripTime || '18 min'}</p>
            <p className="text-[14px] mb-4" style={{ color: '#8A8A8F' }}>Distance: {tripDistance || '5.2 mi'}</p>
            <button onClick={handleCompleteTrip} disabled={!!loading} className="w-full h-[48px] rounded-full text-white text-[16px] font-semibold" style={{ background: '#4CE5B1', border: 'none', cursor: 'pointer' }}>
        ...
        {driverStatus === 'RATE' && ride && (
          <div className="px-4 pb-6 text-center">
            ...
            <p className="text-[16px] font-semibold mb-4" style={{ color: '#242E42' }}>{ride.riderName}</p>
            <button onClick={handleRateRider} disabled={!riderRating || !!loading} className="w-full h-[48px] rounded-full text-white text-[16px] font-semibold" style={{ background: '#4CE5B1', border: 'none', cursor: 'pointer', opacity: riderRating ? 1 : 0.5 }}>
      ...
      {/* Socket error banner */}
      {socketError && (
        <div className="absolute top-[80px] left-4 right-4 z-50 p-2 rounded-lg text-center text-[12px] text-white font-semibold" style={{ background: '#F52D56' }}>
          {socketError}
        </div>
      )}
    </div>
