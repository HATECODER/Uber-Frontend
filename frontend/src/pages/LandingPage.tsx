...
export default function LandingPage() {
    ...
    try {
      ...
      dispatch({
        ...
        payload: {
          ...
          destLng: DESTINATION.lng,
          driverInfo: {
            id: res.driverInfo.id,
            name: res.driverInfo.name,
            phone: res.driverInfo.phone,
            vehicle: res.driverInfo.vehicle,
            licensePlate: res.driverInfo.licensePlate,
            rating: res.driverInfo.rating,
          },
        },
      });
  ...
  return (
    <div className="relative w-[375px] h-[812px] bg-white overflow-hidden">
      ...
      <div className="absolute top-[44px] left-0 right-0 bottom-[106px]" style={{ background: '#e8e4de' }}>
      ...
      <div className="absolute top-[380px] left-4 right-4 flex gap-2 z-10">
        {['Home', 'Office', 'Apartment'].map((label) => (
          ...
          >
            ...
            <span className="text-[12px]" style={{ color: '#3CC585', fontFamily: 'Inter, sans-serif' }}>{label}</span>
          </div>
        ))}
      </div>
      ...
      >
        ...
        <div className="mb-4">
          <div className="flex items-center gap-3 mb-3">
            <div className="w-[12px] h-[12px] rounded-full border-2" style={{ borderColor: '#4CE5B1', background: '#4CE5B1' }} />
            <span className="text-[15px]" style={{ color: '#242E42', fontFamily: '-apple-system, sans-serif' }}>{PICKUP.address}</span>
          </div>
          <div className="ml-[5px] w-[2px] h-[20px] mb-2" style={{ background: '#C8C7CC', borderLeft: '2px dashed #C8C7CC' }} />
          <div className="flex items-center gap-3">
            ...
            <span className="text-[15px]" style={{ color: '#242E42', fontFamily: '-apple-system, sans-serif' }}>{DESTINATION.address}</span>
          </div>
        </div>
        ...
        {error && (
          <p className="text-[13px] text-center mb-3" style={{ color: '#F52D56' }}>{error}</p>
        )}

        <CTAButton label="CONTINUE" onClick={handleBookRide} loading={loading} />
